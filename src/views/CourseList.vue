<template>
  <div class="card p-4 mb-4 shadow-sm border-0">
    ...
    <div v-for="(group, term) in groupedCourses" :key="term" class="mb-5">
      <div
        class="d-flex justify-content-between align-items-center bg-light px-3 py-2 rounded-top"
      >
        <span class="badge bg-info text-dark fs-6">ภาคเรียน {{ term }}</span>
        <div class="text-end fw-semibold">
          หน่วยกิต {{ totalCredits(group) }} เกรดเฉลี่ย
          <span class="text-primary">{{ calculateGPA(group) }}</span>
        </div>
      </div>
      ...
      >
        ...
        <div
          class="text-end fw-bold d-flex align-items-center"
          style="min-width: 80px"
        >
          <span :class="gradeClass(course.grade)" class="me-3">{{
            course.grade
          }}</span>
          <button
...
const totalCredits = (group) => {
  return group.reduce(
    (sum, c) => sum + (gradeToPoint(c.grade) !== null ? c.credit : 0),
    0
  );
};
